...
def score_stability(company_data):
    ...
    if isinstance(insolvency_cases, list) and len(insolvency_cases) > 0:
        first_type = "Unknown"
        if isinstance(insolvency_cases[0], dict):
            first_type = insolvency_cases[0].get("type", "unknown").replace("-", " ").replace("_", " ").title()

        if insolvency.get("has_active_case"):
            adjustment -= 40
            signals.append((f"Active insolvency: {first_type}", -40, "high_risk"))
        else:
            adjustment -= 20
